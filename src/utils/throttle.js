...
// created钩子里面初始化缓存变量：originMap缓存原始事件函数（节流前），throttleMap缓存节流后的事件函数，default缓存默认节流的事件类型，__vnode缓存节流组件包裹的子组件；mounted钩子里面设置include和exclude两个参数的监听事件；destroyed钩子销毁变量。
...
export default {
  ...
  render () {
    ...
    this.__vnode = vnode
    // 针对不同的元素类型设置默认节流事件
    if (vnode.tag === 'el-input') {
      this.default.add('input')
    } else if (vnode.tag === 'el-button') {
